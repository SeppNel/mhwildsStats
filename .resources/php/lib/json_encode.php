...
if (!function_exists('json_encode')) {  
    function json_encode($value) 
    {
        if (is_int($value)) {
	        ...
	        $result = "";
            //for ($i = mb_strlen($value) - 1; $i >= 0; $i--) { // CRASH
	        for ($i = strlen($value) - 1; $i >= 0; $i--) {
                //$mb_char = mb_substr($value, $i, 1); // CRASH
	            $mb_char = substr($value, $i, 1);
                //if (mb_ereg("&#(\\d+);", mb_encode_numericentity($mb_char, $convmap, "UTF-8"), $match)) { // CRASH
	            if (preg_match("/&#(\\d+);/", $mb_chaR, $match)) {
